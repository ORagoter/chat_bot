...
select_query = "SELECT id_answer, answer FROM answers"
...
processed_rows = []
for row in rows:
    id_answer, answer = row
    processed_answer = preprocess_text(answer)
    processed_rows.append((processed_answer, id_answer))

# Выполняем SQL-запрос для загрузки обработанных данных обратно в базу данных
update_query = "UPDATE answers SET answer_processed = %s WHERE id_answer = %s"
